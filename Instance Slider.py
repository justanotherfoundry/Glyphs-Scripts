...
import vanilla
from GlyphsApp import Glyphs, GSInstance, GSCustomParameter, INSTANCETYPESINGLE
...
for ins in f.instances:
	if ins.type != INSTANCETYPESINGLE:
		insList.append(None)
		continue
...
# 5 Autoresize only the first table column. When that table column can no longer be resized, stop autoresizing. Normally you should use one of the sequential autoresizing modes instead.
	...
	def setupSliders(self, insIndex, uiList):
		try:
			...
			if instance.customParameters["InterpolationWeightY"] is not None:
	...
	def revert(self, sender):
		try:
			...
			if insList[index]["WeightY"] is not None:  # if it had WeightY
				if f.instances[index].customParameters["InterpolationWeightY"] is not None:  # if it still has WeightY
				...
				if f.instances[index].customParameters["InterpolationWeightY"] is not None:  # if it does now
					# Glyphs does not redraw after custom parameter deletion,
